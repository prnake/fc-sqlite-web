...
class SqliteDataSet(DataSet):
    def __init__(self, filename, db, bare_fields):
        super(SqliteDataSet, self).__init__(db, bare_fields)
        self._filename = filename

    @property
    def filename(self):
        db_file = datasets[self._filename]._database.database
    ...
    def get_indexes(self, table):
        return datasets[self._filename]._database.get_indexes(table)
    ...
    def get_columns(self, table):
        return datasets[self._filename]._database.get_columns(table)

    def get_foreign_keys(self, table):
        return datasets[self._filename]._database.get_foreign_keys(table)
...
def require_database(fn):
    ...
    def inner(*args, **kwargs):
        ...
        dataset, migrator = load_database(filename)
        ...
        # try:
        res = fn(filename, dataset, migrator, *args, **kwargs)
        # except:
        #     pass
...
def require_database_and_table(fn):
    @wraps(fn)
    def inner(table, *args, **kwargs):
        ...
        # try:
        res = fn(filename, dataset, migrator, table, *args, **kwargs)
        # except:
        #     pass
...
@app.route('/')
@require_database
def index(filename, dataset, migrator):
    return render_template('index.html', sqlite=sqlite3, dataset=dataset, filename=filename)

@app.route('/login/', methods=['GET', 'POST'])
@require_database
def login(filename, dataset, migrator):
    if request.method == 'POST':
        if request.form.get('password') == app.config['PASSWORD']:
            session['authorized'] = True
            return redirect(session.get('next_url') or url_for('index', filename=filename))
        flash('The password you entered is incorrect.', 'danger')
    return render_template('login.html', dataset=dataset, filename=filename)

@app.route('/logout/', methods=['GET'])
@require_database
def logout(filename, dataset, migrator):
    session.pop('authorized', None)
    return redirect(url_for('login', filename=filename))
...
def database_create():
    ...
    if not filename:
        flash('Database filename is required.', 'danger')
...
@require_database
def table_create(filename, dataset, migrator):
    table = (request.form.get('table_name') or '').strip()
    if not table:
        flash('Table name is required.', 'danger')
        dest = request.form.get('redirect') or url_for('index',filename=filename)
        if not dest.startswith('/'):
            dest = '/' + dest
        return redirect(dest)

    dataset[table]
    return redirect(url_for('table_import', filename=filename, table=table))

@app.route('/<table>/')
@require_database_and_table
def table_structure(filename, dataset, migrator, table):
...
@require_database_and_table
def add_column(filename, dataset, migrator, table):
    ...
    if request.method == 'POST':
        if name and col_type in column_mapping:
            ...
            return redirect(url_for('table_structure', filename=filename, table=table))
        else:
...
@require_database_and_table
def drop_column(filename, dataset, migrator, table):
    request_data = get_request_data()
    name = request_data.get('name', '')
    columns = dataset.get_columns(table)
...
@require_database_and_table
def rename_column(filename, dataset, migrator, table):
    request_data = get_request_data()
...
@require_database_and_table
def add_index(filename, dataset, migrator, table):
    request_data = get_request_data()
...
@require_database_and_table
def drop_index(filename, dataset, migrator, table):
    request_data = get_request_data()
    name = request_data.get('name', '')
    indexes = dataset.get_indexes(table)
...
@require_database_and_table
def drop_trigger(filename, dataset, migrator, table):
    request_data = get_request_data()
    name = request_data.get('name', '')
    triggers = dataset.get_triggers(table)
...
@require_database_and_table
def table_content(filename, dataset, migrator, table):
...
@require_database_and_table
def table_query(filename, dataset, migrator, table):
    data = []
    ...
    if request.method == 'POST':
        ...
        if 'export_json' in request.form:
            return export(dataset, table, sql, 'json')
        elif 'export_csv' in request.form:
            return export(dataset, table, sql, 'csv')
...
def export(dataset, table, sql, export_format):
...
@require_database_and_table
def table_import(filename, dataset, migrator, table):
    count = None
    request_data = get_request_data()
        ...
        if not file_obj:
            ...
            try:
                ...
                flash(
                    ...
                    'success')
                return redirect(url_for('table_content', filename=filename, table=table))
...
@require_database_and_table
def drop_table(filename, dataset, migrator, table):
    if request.method == 'POST':
        model_class = dataset[table].model_class
    ...
    return render_template('drop_table.html', filename=filename, dataset=dataset, table=table)
...
def initialize_app(password=None, _db_prefix=None, url_prefix=None):
...
def initialize_database(filename, read_only=False, extensions=None):
    ...
    if filename in datasets:
        return datasets[filename], migrators[filename]
    ...
    if read_only:
        ...
        db = SqliteDatabase('file:%s?mode=ro' % filepath, uri=True)
        try:
            db.connect()
        except OperationalError:
            return None, None
        db.close()
        datasets[filename] = SqliteDataSet(filename, db, bare_fields=True)
    else:
        datasets[filename] = SqliteDataSet(
            filename, 'sqlite:///%s' % filepath, bare_fields=True)

    if extensions:
        for ext in extensions:
            datasets[filename]._database.load_extension(ext)

    migrators[filename] = datasets[filename]._migrator
    datasets[filename].close()
    return datasets[filename], migrators[filename]


def load_database(filename, read_only=False, extensions=None):
    filepath = "/tmp/" + filename
    if not os.path.exists(filepath):
        return None, None
    return initialize_database(filename, read_only, extensions)
